...
| Attachments | Upload, preview, versioning | Local upload **policy** (blocked exts/MIMEs); **disk delete** on remove; optional **`ATTACHMENT_UPLOAD_NOTIFY_URL`** webhook; optional **`ATTACHMENT_RETENTION_DAYS`** daily purge; `ATTACHMENT_ALLOW_EXECUTABLES=1` escape hatch | Improving |
...
| Agile structures | Epics, sprints, stories, points, backlogs | **`Sprint`** + **`sprintId`**; **`workItemType`** / **`storyPoints`**; **`epicTaskId`** (cross-parent epic link) + task detail picker; **sprint** + **epic** filters; **`GET …/epic-rollups`** + epic dropdown **roll-up** hints; **Roadmap** tab (multi-month epic bars); timeline **WBS** + **critical path** toggle + **`isMilestone`** / same-day heuristic (diamond) | Partial |
...
1. **File attachments** — Upload + list + delete + **authenticated download**; abstract storage so **S3-compatible** (or ModelT blob store) can replace local disk without API churn.
...
9. **PM views & agile methodology** — **Shipped (2026-04, see [`pm-views-agile-roadmap.md`](./pm-views-agile-roadmap.md)):** sprint + epic filters; **dedicated Backlog tab** (`/projects/:id/backlog`); **dedicated Sprint board tab** (`/projects/:id/sprint-board`); **Roadmap tab** (`/projects/:id/roadmap`) — multi-month epic timeline; **`Task.epicTaskId`** + roll-ups / epic filter respect linked work; task detail **Epic** link; Burndown (burndown, burnup, velocity); **`SprintMetricSnapshot`** / **`ProjectCfdSnapshot`**; **`GET …/cfd`**, **`GET …/epic-rollups`**; **Flow** tab + **`PROJECT_CFD`** widget; timeline **WBS**, **critical path** toggle, milestones. **Still TBD:** epic dashboard panel, backlog stack-rank beyond section order, richer burnup scope-change narrative.
...
2. ~~**Roadmap #1–2 hardening**~~ — **Done (2026-04):** attachment policy + notify URL + retention cron + delete disk file; custom-field validation, project gate on set, required-before-DONE, custom values on project payload for list/board + **TaskCard/TaskRow** chips. Remaining: S3 storage adapter, formulas, richer scan integration.
...
Next up: deepen **Forms** (file upload on intake, CAPTCHA, rate limits); **#6** saved-view polish; optional **#9** polish (epic dashboard, stack rank). Hardening: **#1** S3-compatible storage, custom-field formulas. **Integrations:** outbound webhook retries / delivery log; Slack OAuth app (vs incoming webhook only).
